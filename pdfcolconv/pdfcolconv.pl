...
my $DEBUG = 0;
my $Id    = '$Id: pdfcolconv.pl 206 2007-11-26 19:29:32Z rose $';
...
# get configuration from "pdfcolconv.conf.pl"
...
logit("FATAL - No Helios installation dir, please specify 'heliosdir' in 'pdfcolconv.conf.pl'!")
...
my $pdfcolconv = $conf->{pdfcolconv} || '/usr/local/pdfColorConvert_CLI/pdfColorConvert';
...
my $pdfColConvSettings = '/var/settings/pdfColorConvert';
...
my $tmpdir = $conf->{tmpdir} . "/pdfcolconv.$PID/";
...
# get pdfColorConvert profile(s)

my ( $profile, $profilekey );

if ($queue) {    # notification script
    my $prof = $conf->{heliosdir} . '/var/spool/qmeta/' . $queue . '/' . 'PDF_COLORCONVERT';
    $profile = $prof if -f $prof && -r $prof;
}
else {           # hotfolder
    if ( $ENV{PDF_COLORCONVERT} ) {
        $profile = defprofile( $ENV{PDF_COLORCONVERT} );
        logit("WARNING - PDF_COLORCONVERT env var '$ENV{PDF_COLORCONVERT}' doesn't point to usable profile") unless -f $profile && -r $profile;
    }
}

unless ($profile) {
	my @profiles = findfile( [dirname($input), dirname($input) . '/..'], qr|^(?!$conf->{generatedcfg}).+\.cfg$| );
	my @iccdata = findfile( [dirname($input), dirname($input) . '/..'], qr(\.(icc|ICC|icm|ICM)$) );
    if( $profiles[0] ) {
		# more than one .cfg file or at least one icc profile
		if( ( scalar @profiles > 1 ) || @iccdata ) {
...
unless( $profile ) {
	logfile( $errlog, logtext( $conf->{errtext} . 'FATAL - cannot find any pdfColorConvert profile, please see docs how to configure this' ) );
	logit( "FATAL - cannot find any pdfColorConvert profile, please see docs how to configure this", 1 );
}
...
my $remote = 1 if $pdfcolconv =~ /^ssh /;
unless ($remote) { logit( "FATAL - not executable '$pdfcolconv'", 1 ) unless -x $pdfcolconv }
...
while (<FD>) { $result .= $_ }
close FD;
if ( $? == -1 ) { logit( "FATAL - failed to execute '$pdfcolconv':\n$!", 1 ) }
elsif ( $? & 127 ) {
    logit(
        sprintf(
            "FATAL - pdfColorConvert died with signal %d, %s coredump\n",
            ( $? & 127 ),
            ( $? & 128 ) ? 'with' : 'without'
        ),
        1
...
if ( $conf->{postprocess} ) {
    ...
    if ( -r $postprocess ) {
        ...
        # make configuration avaible as pdfcolconv_something environment variables
        for my $ckey ( keys %$conf ) {
            if ( ref $conf->{$ckey} eq 'ARRAY' ) { $ENV{ 'pdfcolconv_' . $ckey } = join( ',', @{ $conf->{$ckey} } ) }
            elsif ( ref $conf->{$ckey} eq 'HASH' ) {
                $ENV{ 'pdfcolconv_' . $ckey } = join( ',', each %{ $conf->{$ckey} } );
            }
            else { $ENV{ 'pdfcolconv_' . $ckey } = $conf->{$ckey} }
        }
...
# search for profile file in standard paths pdfColConvSettings = HELIOSDIR/var/settings/pdfColorConvert
# and Profile subdir of pdfColorConvert_CLI installation if profilepath argument is not an abolute path
# return: full path to profile if profile is a plain file and readable, undef otherwise

sub defprofile {
	my $prof = shift;
	if ( $prof !~ m|^/| ) {
		if( -f $conf->{heliosdir} . $pdfColConvSettings . '/' . $prof ) {
			$prof = $conf->{heliosdir} . $pdfColConvSettings . '/' . $prof;
		}
		elsif ( -f dirname($pdfcolconv) . '/Profiles/'  . $prof ) {
			$prof = dirname($pdfcolconv) . '/Profiles/' . $prof;
		}
	}
...
sub generateprofile {
	...
	# if existent include ICC profiles ("file.icc", "file.ICC", "file.icm", "FILE.ICM")
	# assumptions:
	...
	my @iccdata = findfile( [$dir], qr(\.(icc|ICC|icm|ICM)$) );
	if( @iccdata ) {
		for my $iccfile (@iccdata) {
			$DEBUG && logit( "INFO - adding profile '$iccfile'" );
			my @makecmd = ( $pdfcolconv, "-p", "--makeprofiledata", $iccfile, $tmpg );
			system( @makecmd ) && logit( "WARNING - command failed: '@makecmd'" );
		}
	}
...
<General
        Enable="true"
        Hot_Folder="/Volumes/PDF-ColorConvert/IN"
...
=head1 SYNOPSIS
	...
	see documentation by executing "perldoc -F pdfcolconv.pl"
...
=head1 VERSION
	...
	$Id: pdfcolconv.pl 206 2007-11-26 19:29:32Z rose $
